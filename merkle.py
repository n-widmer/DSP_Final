...
This module uses SHA-256 and returns/accepts raw bytes for hashes.

Usage patterns:
- Compute a per-row hash with `compute_row_hash` (deterministic ordering of fields).
- Build a tree from a list of leaf hashes with `build_root_from_hashes`.
- Get an inclusion proof with `get_proof` and verify with `verify_proof`.

This module intentionally keeps I/O out — integrate with your DB layer to fetch
`row_hash` values (BINARY(32) or hex strings) in deterministic order.
"""
...
def get_proof(leaf_hashes: List[bytes], index: int) -> List[Tuple[bytes, bool]]:
    ...
    for level in levels[:-1]:
        # sibling index
        if idx % 2 == 0:
            # sibling is idx+1 if present else duplicate (we add sibling equal to node)
